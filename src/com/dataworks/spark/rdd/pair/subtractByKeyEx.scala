...
/**
  ...
  */
object subtractByKeyEx {
  def main(args: Array[String]): Unit = {
    val conf: SparkConf = new SparkConf().setAppName("subtractByKeyEx").setMaster("local")
  ...
  def subtractEx(sc: SparkContext): Unit = {
    ...
    val subtractedRdd: RDD[(Int, String)] = thisPair.subtractByKey(thatPair)
    subtractedRdd.foreach(x => println(x))
  }
